...
function drawWeather(openweatherData) {
    ...
    elem.currentDateElement.innerHTML = moment.unix(openweatherData.dt).format('DD-MM-YYYY HH:MM:SS');

    console.log('time with moment: ' + moment.unix(openweatherData.dt).format('DD-MM-YYYY HH:MM:SS'));
    ...
    const CURRENT_TEMPERATURE = calculateTemperature(openweatherData.main.temp, ENUMS.TEMPERATURE_SCALES.CELCIUS),
        ...
        CURRENT_TEMPERATURE_WEATHERICON = IMAGE_PATH + openweatherData.weather[0].icon + ".svg";
    ...
    elem.currentStatsSunriseValue.innerText = formatDate(openweatherData.sys.sunrise, 'time', true) + ' -- ' + moment.unix(openweatherData.sys.sunrise).format('HH:MM:SS');
    elem.currentStatsSunsetValue.innerText = formatDate(openweatherData.sys.sunset, 'time', true) + ' -- ' + moment.unix(openweatherData.sys.sunset).format('HH:MM:SS');
}
...
function drawForecast(openweatherDataModified) {
    ...
    openweatherDataModified.forEach((openweatherDataModifiedElement) => {
        ...
        IMAGE_ICON.setAttribute('src', IMAGE_PATH + openweatherDataModifiedElement.weather[0].icon + ".svg");
